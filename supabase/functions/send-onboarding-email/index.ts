  ...
  try {
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
      ...
      user_metadata: {
        name:   candidateName,
        role:   "employee",
        avatar: candidateName.split(" ").map((w: string) => w[0]).slice(0, 2).join("").toUpperCase(),
      },
    });

    if (authError) {
      // User might already exist — don't fail the whole request
    ...
    if (authData?.user) {
      // Get the candidate row for this work email
